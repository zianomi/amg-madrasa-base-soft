...
if(isset($_POST['_chk'])==1) {
    ...
    if(count($errors)==0){
        ...
        if($exm->insertPolicy($data)){
            $_SESSION['msg'] = $tool->Message("succ",$tool->transnoecho("policy_inserted"));
        }
        else{
            $_SESSION['msg'] = $tool->Message("alert",$tool->transnoecho("policy_insert_failed"));
        }
        if(empty($url)){
            Tools::Redir("exam","policies","","");
        }
        else{
            header("Location:" . $url);
        }
                ...
                <div class="span<?php echo $colSize ?>">
                    <table class="table table-bordered table-striped">
                        ...
                        <tr>
                            ...
                            <th class="fonts"><?php $tool->trans("desc") ?></th>
                        ...
                        foreach ($res as $row){
                            ...
                            <tr>
                                ...
                                <?php if($tpl->isCanDelete()){ ?>
                                    <td><a onclick="return confirm('Are you sure you want to delete?');" href="<?php echo Tools::makeLink("exam","policies&del=1&id=".$row['id']."&redir=".$curPageUrl,"","") ?>"><i class="icon-remove"></i></a> </td>
                                <?php } ?>
                            </tr>
                        <?php } ?>
