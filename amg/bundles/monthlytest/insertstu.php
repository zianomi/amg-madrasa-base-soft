...
$hifz_year = (isset($_GET['hifz_year'])) ? $tool->GetInt($_GET['hifz_year']) : '';
...
if(isset($_POST['_chk'])==1) {
    ...
    if(count($errors)==0){
        $test->deleteIDData(array('id' => $stuid, 'branch' => $branch, 'class' => $class, 'section' => $section, 'session' => $session, 'date' => $date));
        $res = $test->insertClassNumbers($vals);
        //$resSyll = $test->insertSyllabus($valSyllabus);
        if($res["status"]){
            $_SESSION['msg'] = $res['msg'];
            $tool->Redir("monthlytest","insertstu","","list");
            exit;
        }
        else{
...
if (isset($_GET['_chk']) == 1) {
    ...
    if (empty($student_id)) {
        echo $tool->Message("alert", $tool->transnoecho("student_id_required"));
        exit;
    }
...
?>
    ...
    <div id="printReady" class="body">
        ...
        //echo '<pre>'; print_r($resNumbs); echo '</pre>';
        ?>
                ...
                <table class="table table-bordered table-striped table-hover flip-scroll">
                    ...
                    foreach ($res as $row) { ?>
                        <tr>
                            ...
                            ?>
                            <td class="fonts">
                                <input value="<?php if(isset($resNumbs[$row['id']][$val["id"]])) echo $resNumbs[$row['id']][$val["id"]]; else echo ""; ?>" onkeyup="CheckValue(this, <?php echo $val['numbers'] ?>)" type="text" name="numbers[<?php echo $row['id']; ?>][<?php echo $val["id"] ?>]" style="width: 40%" maxlength="3" min="0" max="<?php echo $val["numbers"] ?>" pattern="\d+" />
