...
$tblDemo = new AmgCrud();
$tblDemo->setDbTable("jb_monthly_test_subjects");
$tblDemo->setItem($tool->transnoecho("monthly_test_subjects"));
...
if (isset($_GET['_chk']) == 1) {
    ...
    $labels["passing_marks"] = $tool->transnoecho("passing_marks");
    ...
    $tblDemo->omitFieldCompletely("class_id");
    $tblDemo->addValueOnInsert("class_id", $class);

    $tblDemo->omitFieldCompletely("branch_id");
    $tblDemo->addValueOnInsert("branch_id", $branch);
    ...
    $tblDemo->addWhereClause(" WHERE jb_monthly_test_subjects.branch_id = $branch AND jb_monthly_test_subjects.class_id = $class");
