 ...
 * Time: 8:25 PM
 */
class Utf8StringHandling
{


    function create_slug($string){
        return $this->utf8_url_slug($string);
    }


    function stringSummary($str, $search){

        $arr = explode($search, $str);
        $important1 = $arr[0];
        $important2 = $arr[1];

        $final = $important1 . " " . $search . " " . $important2;

        $important = $this->utf8_substr($final, 0, 430);
    ...
    function highlight($str, $search) {
        $pos1 = $this->utf8_strpos($str, $search);
        $len = 250;
        ...
        $total = ($this->utf8_strlen($search)) + $len + $len;

        $string = $this->utf8_substr($str, $pos2, $total);
        return $string;
    }
    ...
    function utf8_url_slug($str = '', $maxl = -1, $trns = false){
        ...
        $str = $this->utf8_clean($str, true); //True == $remove_bom
        $str = $this->utf8_strtolower($str);
        if ($trns && $this->iconv_loaded()) {
            $str = iconv('UTF-8', 'US-ASCII//TRANSLIT//IGNORE', $str);
        }
        if ($this->pcre_utf8_support()) {
        ...
        if ($maxl > 0) {
            $maxl = ( int )$maxl;
            $str = $this->utf8_substr($str, 0, $maxl);
        }
        $str = trim($str, '_-');
        if (!strlen($str)) {
...

    {
        ...
        return count($this->utf8_split($str));
...

    {
        ...
        if (($i = ( int )$code_point) !== $code_point) {
            ...
            if (!($i = ( int )$this->utf8_hex_to_int($code_point))) {
        ...
        if ($this->mbstring_loaded() /*extension_loaded( 'mbstring' )*/) {
...

    {
        ...
        if ($remove_bom) {
            ...
            $str = $this->utf8_str_replace($this->utf8_bom(), '', $str);
...

    {
        ...
        return implode($end, $this->utf8_split($body, $chunklen));
...

    {
        ...
        return ($this->utf8_strlen($str) <= $box_size);
...

    {
        ...
        return array_map('strlen', $this->utf8_split($str));
...

    {
        ...
        return max($this->utf8_chr_size_list($string));
...

    {
        ...
        return implode(array_map('utf8_single_chr_html_encode', $this->utf8_split($str)));
...

    {
        ...
        return $this->is_bom(file_get_contents($file_path, 0, NULL, -1, 3));
...

    {
        ...
        return $this->is_bom(substr($str, 0, 3));
...

    {
        ...
        if (!$this->is_bom(substr($str, 0, 3))) {

            $str = $this->utf8_bom() . $str;
...

    {
        ...
        $array = array_count_values($this->utf8_split($str));
...

    {
        ...
        return implode(array_reverse($this->utf8_split($str)));
...

    {
        ...
        if ((( int )$needle) === $needle && ($needle >= 0)) {
            ...
            $needle = $this->utf8_chr($needle);

        }


        $needle = $this->utf8_clean(( string )$needle);
        ...
        $haystack = $this->utf8_clean($haystack);
        ...
        if ($offset > 0) {
            ...
            $haystack = $this->utf8_substr($haystack, $offset);
        ...
        if (($pos = strpos($haystack, $needle)) !== false) {
            ...
            return ($offset > 0 ? $offset : 0) + $this->utf8_strlen($left);
...

    {
        ...
        return $this->utf8_chr(max($this->utf8_codepoints($arg)));
...

    {
        ...
        return $this->utf8_chr(min($this->utf8_codepoints($arg)));
...

    {
        ...
        return $this->utf8_int_to_hex($this->utf8_ord($chr), $pfix);
...

    {
        ...
        return count(explode('-', $this->utf8_url_slug($str)));
...

    {
        ...
        if ($offset || $length) {
            ...
            $haystack = $this->utf8_substr($haystack, $offset, $length);
...

    {
        ...
        } else if (!($start = ( int )$this->utf8_hex_to_int($var1))) {

            //if not u+0000 style codepoint


            if (!($start = $this->utf8_ord($var1))) {
        ...
        } else if (!($end = ( int )$this->utf8_hex_to_int($var2))) {

            //if not u+0000 style codepoint


            if (!($end = $this->utf8_ord($var1))) {
...

    {
        ...
        return $this->utf8_chr_map($callback, $str);
...

    {
        ...
        return $this->utf8_substr($string, $pos, 1);
...

    {
        ...
        $array = $this->utf8_codepoints($str);
...

    {
        ...
        $input_length = $this->utf8_strlen($input);


        if (is_int($pad_length) && ($pad_length > 0) && ($pad_length >= $input_length)) {

            $ps_length = $this->utf8_strlen($pad_string);
            ...
            switch ($pad_type) {
                ...
                case STR_PAD_BOTH:
                    ...
                    $pre = $this->utf8_str_repeat($pad_string, ( int )ceil($diff / $ps_length / 2));

                    $pre = $this->utf8_substr($pre, 0, ( int )$diff / 2);

                    $post = $this->utf8_str_repeat($pad_string, ( int )ceil($diff / $ps_length / 2));

                    $post = $this->utf8_substr($post, 0, ( int )ceil($diff / 2));
                ...
                default:
                    ...
                    $post = $this->utf8_str_repeat($pad_string, ( int )ceil($diff / $ps_length));

                    $post = $this->utf8_substr($post, 0, $diff);
...

    {
        ...
        if ($offset > 0) {
            ...
            $haystack = $this->utf8_substr($haystack, 0, $offset);
...

    {
        ...
        $string = $this->utf8_clean($string); //Cleanup is necessary here, or trim functions may break
...

    {
        ...
        $string = $this->utf8_ltrim($string, $chrs);


        return $this->utf8_rtrim($string, $chrs);
...

    {
        ...
        $this->utf8_trim_util($string, $chrs);


        $s_look = $this->utf8_split($string);
        ...
        return $this->utf8_substr($string, $count);
...

    {
        ...
        return $this->utf8_substr($string, 0, $len);
...

    {
        ...
        if (empty($table)) {
            ...
            $table = array_flip($this->utf8_case_table());
...

    {
        ...
        return $this->utf8_strtoupper($this->utf8_substr($str, 0, 1)) . $this->utf8_substr($str, 1);
...

    {
        ...
        return $this->utf8_strtolower($this->utf8_substr($str, 0, 1)) . $this->utf8_substr($str, 1);
...

    {
        ...
        foreach ($this->utf8_ws() as $ws) {
            ...
            while (true) {
                ...
                if (($pos = $this->utf8_strpos($str, $ws, $pos + 1)) === false) {
                ...
                $str = $this->utf8_substr($str, 0, $pos + 1) . $this->utf8_strtoupper(utf8_substr($str, $pos + 1, 1)) . $this->utf8_substr($str, $pos + 2);
...

    {
        ...
        return $this->utf8_strpos($this->utf8_strtolower($haystack), $this->utf8_strtolower($needle), $offset);
...

    {
        ...
        return $this->utf8_strrpos($this->utf8_strtolower($haystack), $this->utf8_strtolower($needle), $offset);
